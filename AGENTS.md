...
- **Routes**: `/` (index, open), `/config` (config dashboard, admin-gated), `/audit` (BorgAUDIT commit-history page, admin-gated), `/progress` (forced-backup progress, open), `/files/` (static file server rooted at `config.Path`, admin-gated), `/force` (manual trigger, admin-gated), `/approve` (single-commit approval toggle, admin-gated), `/approve-all` (bulk approve, admin-gated), `/auth/login` (admin login POST), `/auth/logout` (session revoke POST), `/auth/state` (mode + lock JSON polled by nav-bar JS), `/auth-hash` (credential generator bootstrap page), `/favicon.ico`.
- **Middleware**: page-render routes (`/`, `/config`, `/audit`, `/progress`, `/files/`) are wrapped with `addSecurityHeader`. `/config`, `/audit`, `/files/`, `/force`, `/approve`, and `/approve-all` are additionally wrapped with `requireAdmin` (or `requireAdminFiles` for the static file server). When credentials are armed (`OPN_AUTH_HASH` + `OPN_AUTH_SALT` set and valid) and no live admin session is present, `requireAdmin` redirects to `audit?auth=locked` and `requireAdminFiles` redirects to `config?auth=locked`. When no credentials are configured the middleware is a pass-through so the monitoring-only view remains usable without login.
...
The WebUI **always starts in monitoring-only view mode** — the hive status dashboard (`/`) and forced-backup progress page (`/progress`) are always readable without login. The `[ Authenticate ]` nav-bar button unlocks admin mode per browser session.

- **Monitoring mode** (green `MONITORING` badge): the index (`/`) and progress (`/progress`) pages are readable. Config-file downloads (`/files/`), the config dashboard (`/config`), the audit page (`/audit`), forced backup trigger (`/force`), and approval actions (`/approve`, `/approve-all`) are locked — `requireAdmin`/`requireAdminFiles` redirect unauthenticated requests to `audit?auth=locked` (or `config?auth=locked` for `/files/`). Download buttons render as greyed-out locked controls that open the auth info dialog on click.
- **Admin mode** (red/yellow `ADMIN` badge): per-browser-session state unlocked by submitting the admin password via the nav-bar dialog; enables config downloads, config dashboard, audit page with full diffs, forced backup, and approval actions.
- **Startup validation** (`authInit`, called from `Setup()`): login is armed only when `OPN_AUTH_HASH` + `OPN_AUTH_SALT` are both present, non-empty, non-whitespace, and valid — the hash must carry the `$` separator with both key halves decoding to exactly 64 bytes (keylen), and the salt must base64-decode to >= 8 raw bytes. Invalid content keeps auth fully disabled (`[AUTH][DISABLED]`), the login dialog is never rendered, `authCheckPassword` refuses every attempt, and `requireAdmin`/`requireAdminFiles` are pass-throughs (no gate to enforce).
- **Middleware** (`auth-http.go`): `requireAdmin(next)` checks `authCredentialsEnabled()` and `authIsAdmin(r)` — if credentials are armed but no live admin session exists, the request is redirected to `audit?auth=locked` (HTTP 303). `requireAdminFiles(next)` does the same but redirects to `config?auth=locked`. Both are pass-throughs when credentials are not configured.
...
- **OpenAI-compatible fallback** (opt-in via `OPENAI_DESC_URL`, required; `OPENAI_DESC_MODEL`, optional, defaults to `gpt-4o-mini`; `OPENAI_DESC_TOKEN`, optional). When Ollama is unset or fails, the same enriched diff and prompt go to `<OPENAI_DESC_URL>/chat/completions` (two-message chat, `stream=false`). Token sent as `Authorization: Bearer <token>` when set. Same retry/fallback contract via shared `generateWithRetry`. The `reviewPending` atomic banner is armed when either backend is enabled.
...
```
<OPN_PATH>/
  .gitignore                         # auto-created: ignores .archive, CONFIG*, Logs, approval.db*
  <server>/
...
`git.go` manages the storage folder as a git repo (opt-in via `OPN_GIT_ENABLE`). `gitInit` opens-or-inits the repo and reconciles `.gitignore` at startup. `gitCheckIn` runs per tick when `config.dirty` is set: `os.Chdir(config.Path)`, status fast-path, `wtree.Add(".")`, commit. The commit author is `_authorName` (`OPNBORG-AUTO-COMMIT`) unless a model authored the message, in which case `authorFromCommitMessage` (`ollama.go`) uses a sanitised version of the headline. `gitPush` pushes via `ssh.NewPublicKeysFromFile` from `OPN_GIT_SSH_KEY`, with host key verification via `OPN_GIT_SSH_HOSTKEY` fingerprint matching (or `InsecureIgnoreHostKey` when unset). `gitGC` runs a best-effort per-tick repack (`pack.deltaWindow` raised to `_aggressivePackWindow`, 250) and prunes unreachable objects. All git operations use `go-git` — no external `git` binary.
...
- `renderOpenAIPanel` shows parsed `OPENAI_DESC_URL` / `OPENAI_DESC_MODEL` / `OPENAI_DESC_TOKEN` plus a live probe (`openaiHealthCheck`) that GETs `<OPENAI_DESC_URL>/models`. Token shown as a set/not-set pill.
