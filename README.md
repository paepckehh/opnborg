...
The WebUI ships with a two-mode access model. On startup opnborg runs in
**monitoring mode** (green badge in the top nav bar): the full hive dashboard
is readable without any authentication, but config-file downloads
(`current.xml` / `archive` buttons, greyed out with a hint) and the BorgAUDIT
approval actions are locked. An **[ Authenticate ]** button sits in the nav
bar between the mode badge and the version pill.

Setting both `OPN_AUTH_HASH` and `OPN_AUTH_SALT` arms admin mode: submitting
the matching password via the nav-bar dialog unlocks a per-browser session
(12 h sliding TTL, HttpOnly SameSite=Strict cookie) with a red/yellow **ADMIN**
badge. Admin mode enables the config downloads and the approve / approve-all
actions on the BorgAUDIT page.
...
The **Authentication** tile on the config dashboard shows the current mode and
a *Create Authentication Env Vars* button: enter the password you want to use,
...
| Variable | Default | Description |
| --- | --- | --- |
| `OPN_AUTH_HASH` | _empty_ | Argon2id admin credential, format `<base64-key>$<base64-key>` (generated by the WebUI credential generator); arming requires `OPN_AUTH_SALT` too |
| `OPN_AUTH_SALT` | _empty_ | Base64-encoded random salt (16 raw bytes) belonging to `OPN_AUTH_HASH` |
